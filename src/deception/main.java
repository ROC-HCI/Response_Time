...
public class main {
	...
	public static void main(String[] args) throws IOException, UnsupportedAudioFileException, LineUnavailableException {
		...
		//Make sure inputs are legal 
		if(args.length < 3) throw new IllegalArgumentException(); //if too few arguments inputed 
		if(args.length >= 4) {
			if(Double.parseDouble(args[3]) > sents.get(sents.size() - 1).end) //if start time > conversation length
				throw new IllegalArgumentException(); 
		}
		if(args.length == 5){
			if(Double.parseDouble(args[4]) > sents.get(sents.size() - 1).end) //if end time > conversation length
				throw new IllegalArgumentException(); 
			if(Double.parseDouble(args[3]) > Double.parseDouble(args[4])) //if start time > end time
				throw new IllegalArgumentException(); 
		}
		
			
		//Generate Output file
		if(args.length == 3) generateOutput(args[2], 0, sents.get(sents.size() - 1).end);
		if(args.length == 4) generateOutput(args[2], Double.parseDouble(args[3]), sents.get(sents.size() - 1).end);
		if(args.length == 5) generateOutput(args[2], Double.parseDouble(args[3]), Double.parseDouble(args[4]));
		
		
		
		
//		AudioInputStream descAudio = AudioSystem.getAudioInputStream(new File("/Users/test/Desktop/2016-03-02_14-03-04-887-RUTHEH.wav"));
//		AudioInputStream intAudio = AudioSystem.getAudioInputStream(new File("/Users/test/Desktop/2016-03-02_14-03-04-887-Simplyme.wav"));

		
		System.out.println(getAvgRepTime(sents));
